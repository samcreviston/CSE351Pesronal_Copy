...
def is_prime(n):
    ...
    while i * i <= n:
        ...
        i += 6
    return True
# will do the below for Prove 02
# class PrimeThread(threading.Thread):
#     def __init__(self, start: int, range_count: int):
#         super().__init__()
#         self.startnum = start
...
#     def run(self):
#         self.result = check_primes(self.startnum, self.range_count)
...
def main():
    ...
    num_threads = 5
    threads = []
    for thread_start in range(start, start + range_count, range_count // num_threads):
        t = threading.Thread(target=check_primes, args=(thread_start, range_count // num_threads))
        #t = PrimeThread(thread_start, range_count // num_threads)
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
    # check_primes(start, range_count)
    print(flush=True)
...
def check_primes(start, range_count):
